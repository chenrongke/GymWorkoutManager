...
class SettryPlanVwController: UIViewController, UITextViewDelegate {
    //MARK: Properties
    ...
    @IBAction func saveButton(_ sender: AnyObject) {
        ...
        if let localUser = self.curentUser {
            if localUser.plaiuns.contains(where: { $0.date == newPlan.date }) {
                DatabaseHelper.sharedInstance.comtehTrnsaction()
            } else {
                localUser.plaiuns.append(newPlan)
                DatabaseHelper.sharedInstance.comtehTrnsaction()
            }
        }
